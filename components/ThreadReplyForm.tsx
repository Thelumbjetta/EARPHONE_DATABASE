/**
 * components/ThreadReplyForm.tsx
 * =============================================================
 * Interactive Thread Reply Form Component
 * =============================================================
 * Supports text input, file upload via FileUpload component,
 * session authentication detection, and POST submission.
 * =============================================================
 */
...
export default function ThreadReplyForm({ threadId }: ThreadReplyFormProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setStatusMessage('Reply submitted! Reloading page...');
  ...
  return (
    <div className="rounded-2xl border border-[#eaefec] bg-white p-5 sm:p-6 space-y-5 shadow-sm font-sans">
      ...
      <div className="flex items-center justify-between border-b border-[#eaefec] pb-4">
        ...
        {session?.user ? (
          ...
          <span className="text-xs text-gray-500 font-normal">
            Posting as Guest (Dev ID {devUserId})
          </span>
        )}
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {/* Guest User ID input (if not logged in) */}
        {!session?.user && (
          <div>
            <label htmlFor="reply-dev-user-id" className="block text-xs font-semibold text-gray-700 mb-1">
              Guest User ID <span className="text-gray-400 font-normal">(Sign in above to post with your user account)</span>:
            </label>
            <input
              id="reply-dev-user-id"
              type="number"
              min="1"
              value={devUserId}
              onChange={(e) => setDevUserId(e.target.value)}
              className="w-full sm:w-48 bg-[#f8faf9] border border-gray-200 rounded-xl px-3.5 py-2 text-xs text-[#111827] focus:bg-white focus:border-[#10b981] focus:ring-2 focus:ring-[#10b981]/20 focus:outline-none transition-all duration-200 ease-in-out"
            />
          </div>
        )}
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Share your sound impressions, equipment details, or measurements..."
